...
import { likePost, unlikePost, deletePost } from '../../utils/api/post';
...
function ProfilePostCard(props) {
  ...
  const [postInfo, setPostInfo] = useState(() => ({
    // console.log('Formatting post info');
    ...post,
    like_count_str: nFormatter(post.likeInfo.length, 1),
  }));

  const ifUserLikedPost = () => post.likeInfo.filter((i) => `${i}` === `${postInfo.belongUserId}`).length;
  ...
  const clickHeartHandler = async () => {
    ...
    if (heartClicked) {
      // Unlike the post
      const res = await unlikePost(postInfo._id, postInfo.belongUserId);
      ...
      // Like a post
      const res = await likePost(postInfo._id, postInfo.belongUserId);
      const editedPost = res.data.response;
  ...
  // TODO: Fetch Post User Info (Nickname, avatar)
        ...
        <div className="">
          <div className="d-flex align-items-center justify-content-between">
            <div className="d-flex align-items-center">
              ...
              />
              {/* User name */}
              <p className="text-dark my-0 ms-2 text-justify">{username}</p>
            </div>
            <DropdownButton
              align="end"
              title=""
              ...
              variant="light"
            >
              <Dropdown.Item eventKey="edit" onClick={() => handleEditClicked(postInfo)}>Edit</Dropdown.Item>
              <Dropdown.Divider />
              <Dropdown.Item
                eventKey="delete"
                onClick={async () => {
                  // TODO: change delete post to mangodb
          ...
          <div className="d-flex justify-content-between mx-3 my-2">
            ...
            <div className="d-flex justify-content-between align-items-center">
              ...
              <p className="text-dark my-0 fw-bold">{postInfo.likeInfo.length}</p>
            </div>
          </div>
        </div>
      </div>
